...
class BigEndianByteReaderTests: XCTestCase {
    ...
    func testByte() {
        ...
        let reader = BigEndianByteReader(source: Data([randomByte, UInt8.min, UInt8.max]))
    ...
    func testIsFinished() {
        let reader = BigEndianByteReader(source: TestHelper.byteData)
        XCTAssertFalse(reader.isFinished)
        reader.offset = 9
        XCTAssertTrue(reader.isFinished)
    }
    ...
    func testBytes() {
        ...
        let reader = BigEndianByteReader(source: Data(bytes))
        XCTAssertEqual(reader.bytes(count: 0), [])
        XCTAssertEqual(reader.bytes(count: count), bytes)
    }

    func testIntFromBytes() {
        var reader = BigEndianByteReader(source: TestHelper.byteData)
        ...
        if MemoryLayout<Int>.size == 8 {
            reader = BigEndianByteReader(source: Data([0x7F, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                                                     0x80, 0, 0, 0, 0, 0, 0, 0]))
        } else if MemoryLayout<Int>.size == 4 {
            reader = BigEndianByteReader(source: Data([0x7F, 0xFF, 0xFF, 0xFF, 0x80, 0, 0, 0]))
        } else {
    ...
    func testUint64() {
        var reader = BigEndianByteReader(source: TestHelper.byteData)
        XCTAssertEqual(reader.uint64(), 0x0001020304050607)
        reader = BigEndianByteReader(source: Data(Array(repeating: 0xFF, count: 8)))
        XCTAssertEqual(reader.uint64(), UInt64.max)
        reader = BigEndianByteReader(source: Data(Array(repeating: 0, count: 8)))
    ...
    func testUint64FromBytes() {
        ...
        XCTAssertFalse(reader.isFinished)
        reader = BigEndianByteReader(source: TestHelper.byteData)
    ...
    func testUint32() {
        var reader = BigEndianByteReader(source: TestHelper.byteData)
        XCTAssertEqual(reader.uint32(), 0x00010203)
        reader = BigEndianByteReader(source: Data(Array(repeating: 0xFF, count: 4)))
        XCTAssertEqual(reader.uint32(), UInt32.max)
        reader = BigEndianByteReader(source: Data(Array(repeating: 0, count: 4)))
    ...
    func testUint16() {
        ...
        reader = BigEndianByteReader(source: Data(Array(repeating: 0xFF, count: 2)))
        XCTAssertEqual(reader.uint16(), UInt16.max)
        reader = BigEndianByteReader(source: Data(Array(repeating: 0, count: 2)))
    ...
    func testNonZeroStartIndex() {
        let reader = BigEndianByteReader(source: TestHelper.byteData[1...])
